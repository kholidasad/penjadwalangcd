...
/*********************************************************************************************/
	function pekerja(){
		...
		$data['page_title'] = 'Data Pekerja';		
		$url = base_url() . 'web/pekerja/';
	...
	function pekerja_add(){
		...
		$data['page_title'] = 'Data Pekerja Add';	
		$data['rs_mk'] = $this->m_matakuliah->get();
	...
	function pekerja_edit($kode){
		...
		$data['page_title'] = 'Data Pekerja Edit';	
		$data['rs_dosen'] = $this->m_dosen->get_by_kode($kode);
		$this->render_view($data);	
	}
	
	function pekerja_delete($kode){
		...
		redirect(base_url() . 'web/pekerja','reload');
	}
	
	
	function pekerja_search(){

		$search_query = $this->input->post('search_query');

		$data['rs_dosen'] = $this->m_dosen->get_search($search_query);
		$data['page_title'] = 'Cari Pekerja';
		$data['page_name'] = 'dosen';	
...
/*********************************************************************************************/
	...
	function posisi_add(){
		...
		$data['page_title'] = 'Tambah Posisi';	
		
		$this->render_view($data);	
	}
	
	function posisi_edit($kode){
		$data = array();
		...
		$data['page_title'] = 'Edit Posisi';	
		$data['rs_mk'] = $this->m_matakuliah->get_by_kode($kode);
		$this->render_view($data);	
	}
	
	function posisi_delete($kode){
		...
		redirect(base_url() . 'web/posisi','reload');
	}
	
	function posisi_search(){
		$search_query = $this->input->post('search_query');

		$data['rs_mk'] = $this->m_matakuliah->get_search($search_query);
		$data['page_title'] = 'Cari Posisi';
		$data['page_name'] = 'matakuliah';	
		$data['search_query'] = $search_query;
		$data['start_number'] = 0;
...
/***********************************************************************************************/
	...
	function wilayah_add(){
		...
		$data['page_title'] = 'Tambah Data Wilayah';	
		
		$this->render_view($data);	
	}
	
	function wilayah_edit($kode){
		/*kode,nama,kapasitas,jenis*/
		$data = array();
		if(!empty($_POST)){
		...
		$data['page_title'] = 'Edit Data Wilayah';	
	...
	function wilayah_search(){
		$search_query = $this->input->post('search_query');

		$data['rs_ruang'] = $this->m_ruang->get_search($search_query);
		$data['page_title'] = 'Cari Wilayah';
		$data['page_name'] = 'ruang';	
		$data['search_query'] = $search_query;
		//$data['start_number'] = 0;
...
/***************************************************************************/	
	...
	function excel_report(){
        ...
        $objPHPExcel->getProperties()->setTitle("Penjadwalan")->setDescription("penjadwalan");
        ...
        //     $col++;
        // }
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, 1, 'Hari');
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, 1, 'Jam');
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, 1, 'Pekerja');
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, 1, 'Tempat');
		
		// Fetching the table data
        $row = 2;
        foreach($query->result() as $data)
        {
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $data->hari);
        	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data->jam_kuliah);
        	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data->dosen);
        	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data->ruang);
            $row++;
        }
        ...
        header('Content-Disposition: attachment;filename="jadwal-'.date('dMy').'.xlsx"');
